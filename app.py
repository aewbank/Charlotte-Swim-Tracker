...
# --- 2. CSS (Single Line to avoid Syntax Errors) ---
st.set_page_config(page_title="Swim Tracker", layout="centered")
st.markdown("<style>.stApp {background-color: white !important;} div[data-testid='stTable'] {background-color: white !important;} div[data-testid='stTable'] th, div[data-testid='stTable'] td, .stTable td, .stTable th {color: black !important; background-color: white !important; font-weight: 500 !important;} [data-testid='stSidebar'] {background-color: #111111 !important;} [data-testid='stSidebar'] * {color: white !important;} .header-box {background-color: #0056b3; padding: 20px; border-radius: 15px; color: white !important; text-align: center; margin-bottom: 20px;}</style>", unsafe_allow_html=True)

# --- 3. HEADER ---
st.markdown(f'<div class="header-box"><h1 style="color:white;">🏊 {name.upper()} TRACKER</h1></div>', unsafe_allow_html=True)
